...
def log_peripheral_tables(
    ...
    if isinstance(peripheral_tables, dict):
        for name, table in peripheral_tables.items():
            log_table(table, f"Peripheral.{name}")
    else:
        for id, table in enumerate(peripheral_tables):
            log_table(table, f"Peripheral.{id}")
...
def log_table(
    ...
    if isinstance(table, (getml.DataFrame, getml.data.View)):
        _log_dataframe_or_view(table, context)
    elif isinstance(table, getml.data.Subset):
        _log_subset(table, f"{context}")
...
def _log_subset(subset: getml.data.Subset, context: str) -> None:
    _log_dataframe_or_view(subset.population, f"{context}.Population")

    for name, table in subset.peripheral.items():
        _log_dataframe_or_view(table, f"{context}.Peripheral.{name}")
